...
const decimalString = (value: string | number): string => String(value);

const mapAddress = (address: SearchRequest["start"]["address"]): OndcAddress => ({
  name: address.name,
...
export const mapSearchRequestToOndc = (
  ...
  const intent: OndcSearchRequest["message"]["intent"] = {
    ...
    fulfillment: {
      ...
      start: {
        location: { gps: request.start.gps, address: mapAddress(request.start.address) },
        authorization: { type: request.authorization.startType },
      },
      end: {
        location: { gps: request.end.gps, address: mapAddress(request.end.address) },
  ...
  if (request.schedule) {
    intent.provider = {
      time: {
        ...
        schedule: request.schedule.holidays?.length
          ? { holidays: request.schedule.holidays }
          : undefined,
        range:
          request.schedule.rangeStart || request.schedule.rangeEnd
            ? { start: request.schedule.rangeStart, end: request.schedule.rangeEnd }
            : undefined,
      },
    };
  }
