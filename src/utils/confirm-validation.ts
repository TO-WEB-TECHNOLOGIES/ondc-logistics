...
const validateBasicOrder = (v: unknown, p = "message.order") => {
  ...
  items.forEach((x: any, i: number) => { const z = record(x, p + ".items[" + i + "]"); str(z.id, p + ".items[" + i + "].id"); str(z.fulfillment_id, p + ".items[" + i + "].fulfillment_id"); if (!fids.has(z.fulfillment_id)) throw new ConfirmValidationError("does not reference an order fulfillment", p + ".items[" + i + "].fulfillment_id"); str(z.category_id, p + ".items[" + i + "].category_id"); });
  fulfillments.forEach((x: any, i: number) => { const z = record(x, p + ".fulfillments[" + i + "]"); str(z.type, p + ".fulfillments[" + i + "].type"); });
  ...
  const payment = record(o.payment, p + ".payment"); str(payment.type, p + ".payment.type"); str(payment.collected_by, p + ".payment.collected_by");
  if (payment["@ondc/org/collection_amount"] === undefined && payment.paid_amount === undefined) throw new ConfirmValidationError("payment amount is required", p + ".payment");
  return o;
};
const validateWorkbenchFields = (o: any, p: string) => {
  const categories = ["Express Delivery", "Standard Delivery", "Immediate Delivery", "Next Day Delivery", "Same Day Delivery", "Instant Delivery"];
  o.items.forEach((item: any, i: number) => { if (!categories.includes(item.category_id)) throw new ConfirmValidationError("must be a valid ONDC delivery category", p + ".items[" + i + "].category_id"); });
};
...
export const parseConfirmRequest = (value: unknown): ConfirmRequest => {
  ...
  if (x.context !== undefined) {
    ...
    if (c.action !== undefined && c.action !== "confirm") throw new ConfirmValidationError("must be confirm", "context.action");
    result.context = {
      ...(c.transaction_id !== undefined ? { transaction_id: str(c.transaction_id, "context.transaction_id") } : {}),
      ...(c.message_id !== undefined ? { message_id: str(c.message_id, "context.message_id") } : {}),
    };
  }
  if (x.message !== undefined) {
    const m = record(x.message, "message");
    if (m.order !== undefined) result.message = { order: record(m.order, "message.order") };
  }
  if (x.order !== undefined) result.order = record(x.order, "order");

  return result;
};
export const validateConfirmPayload = (value: OndcConfirmRequest): void => { if (value.context.action !== "confirm") throw new ConfirmValidationError("must be confirm", "context.action"); validateBasicOrder(value.message.order); validateWorkbenchFields(value.message.order, "message.order"); requireAcceptance(value.message.order); };
export const parseOnConfirmResponse = (value: unknown): OndcOnConfirmResponse => {
