 ...
 * usable context (e.g. a malformed callback). An array body (workbench sends
 * /on_issue_status wrapped in one) echoes its first item's context.
 */
export const syncResponseContext = (body: unknown) => {
  const source = Array.isArray(body) ? body[0] : body;
  const context = (source as { context?: unknown } | undefined)?.context;
