...
const withBapAcceptance = (tags: unknown) => {
  ...
  if (existing) {
    existing.list = existing.list.filter((x: any) => x.code !== "accept_bpp_terms");
...
const itemIdentity = (items: unknown) => Array.isArray(items) ? items.map((x: any) => ({ id: x?.id, fulfillment_id: x?.fulfillment_id, category_id: x?.category_id })) : items;

const requireSame = (incoming: any, initialized: any, path: string) => {
  if (incoming === undefined || incoming === null) throw new ConfirmValidationError("is required in confirm request", path);
  if (!same(incoming, initialized)) throw new ConfirmValidationError("does not match initialized transaction", path);
};

const requireSameIfProvided = (incoming: any, initialized: any, path: string) => {
  if (incoming === undefined || incoming === null) return;
  if (!same(incoming, initialized)) throw new ConfirmValidationError("does not match initialized transaction", path);
};

const validateAgainstInit = (incoming: any, initialized: any) => {
  requireSameIfProvided(incoming.provider?.id, initialized.provider?.id, "message.order.provider.id");
  requireSameIfProvided(incoming.provider?.locations?.map((x: any) => x?.id), initialized.provider?.locations?.map((x: any) => x?.id), "message.order.provider.locations");
  requireSameIfProvided(itemIdentity(incoming.items), itemIdentity(initialized.items), "message.order.items");
  requireSameIfProvided(incoming.fulfillments?.map((x: any) => ({ id: x?.id, type: x?.type })), initialized.fulfillments?.map((x: any) => ({ id: x?.id, type: x?.type })), "message.order.fulfillments");
  requireSameIfProvided(incoming.quote, initialized.quote, "message.order.quote");
  requireSameIfProvided(incoming.billing, initialized.billing, "message.order.billing");
  requireSameIfProvided(incoming.payment, initialized.payment, "message.order.payment");
  requireSameIfProvided(incoming["@ondc/org/linked_order"], initialized["@ondc/org/linked_order"], "message.order.@ondc/org/linked_order");
};
...
export class ConfirmService {
  ...
  async createConfirm(input: ConfirmRequest): Promise<ConfirmResponse> {
    console.log("[confirm.service] createConfirm invoked", { initTransactionId: input.initTransactionId, hasOrderSupplement: Boolean(input.message?.order ?? input.order) });
    ...
    const initializedState = { ...state.init, message: { ...state.init.message, order: initialized } } as any;
    const suppliedOrder = (input.message?.order ?? input.order ?? {}) as any;
    const initTransactionId = input.initTransactionId;

    if (initTransactionId !== state.initTransactionId || initTransactionId !== state.init.context.transaction_id) {
      throw new ConfirmValidationError("must match initialized transaction_id", "initTransactionId");
    }

    if (input.context?.transaction_id !== undefined && input.context.transaction_id !== initTransactionId) {
      throw new ConfirmValidationError("must match initialized transaction_id", "context.transaction_id");
    }
    if (input.context?.message_id === state.init.context.message_id) {
      throw new ConfirmValidationError("must be unique for confirm", "context.message_id");
    }
    if (!initialized.quote) throw new ConfirmValidationError("initialized order quote is required", "message.order.quote");

    const requestItems = suppliedOrder?.items;
    const initializedItems = initializedState?.message?.order?.items;
    const itemsMatch = same(itemIdentity(requestItems), itemIdentity(initializedItems));
    console.log("CONFIRM initTransactionId:", initTransactionId);
    console.log("CONFIRM request items:", JSON.stringify(requestItems, null, 2));
    console.log("INITIALIZED transaction items:", JSON.stringify(initializedItems, null, 2));
    console.log("INITIALIZED transaction:", initializedState?.context?.transaction_id);
    console.log({ requestItems, initializedItems, itemsMatch, requestComparableItems: itemIdentity(requestItems), initializedComparableItems: itemIdentity(initializedItems) });
    validateAgainstInit(suppliedOrder, initialized);

    if (suppliedOrder.id !== undefined && state.orderId !== undefined && suppliedOrder.id !== state.orderId) {
      throw new ConfirmValidationError("must reuse initialized confirm order.id", "message.order.id");
    }
    ...
    const payload: OndcConfirmRequest = {
      context: { ...state.init.context, action: "confirm", transaction_id: initTransactionId, message_id: input.context?.message_id as string ?? randomUUID(), timestamp: now },
      message: { order },
    };
    validateConfirmPayload(payload);
    console.log("[confirm.service] final ONDC /confirm payload", JSON.stringify(payload, null, 2));
    const created = await this.dependencies.repository.create(payload, state.initTransactionId);
    if (!created) {
      console.log("[confirm.service] idempotent /confirm retry", { transactionId: payload.context.transaction_id, messageId: payload.context.message_id, orderId });
