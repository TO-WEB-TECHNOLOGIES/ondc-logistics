...
const order = {
  ...
  fulfillments: [
    {
      ...
      start: {
        ...
        contact: { phone: "9000000000", email: "store@example.com" },
        time: { duration: "PT15M" },
        person: { name: "Pickup Agent" },
      },
      end: {
        location: {
          gps: "12.2,77.2",
        ...
        contact: { phone: "9111111111", email: "buyer@example.com" },
        time: { duration: "PT30M" },
  ...
  payment: {
    "@ondc/org/collection_amount": "59.00",
    collected_by: "BPP",
    type: "ON-FULFILLMENT",
    "@ondc/org/settlement_details": [
      { settlement_counterparty: "lbnp", settlement_type: "upi" },
    ],
  },
...
test("confirm snapshot contains final ONDC fulfillment and billing paths", async () => {
  ...
  const service = new ConfirmService({
    repository: {
      async loadInitialized() {
        return {
          initTransactionId: "TX-1",
          init: {
            context: {
          ...
          onInit: {
            context: { action: "on_init", transaction_id: "TX-1" },
            message: { order },
          } as any,
        };
      },
    ...
    transport: {
      async sendSearch() {},
      async sendInit() {},
      async sendConfirm(payload: any) {
        sent = payload;
      },
    },
  });
  await service.createConfirm({
    initTransactionId: "TX-1",
    order: {
      ...
      "@ondc/org/linked_order": order["@ondc/org/linked_order"],
    },
  });
  ...
  assert.deepEqual(
    ...
    order.fulfillments[0].start.location,
  );
  assert.deepEqual(
    fulfillment.start.contact,
    order.fulfillments[0].start.contact,
  );
  assert.deepEqual(
    fulfillment.end.location,
    order.fulfillments[0].end.location,
  );
